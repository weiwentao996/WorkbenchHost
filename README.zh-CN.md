...
WorkbenchHost 是一个独立运行的编辑器式 Windows 宿主。它使用真实文件和真实文件夹，不生成伪造代码文件；所有应用适配器共用同一个真实的 `db.go` 作为启动入口。

## 快速开始

1. 双击 `Start Workbench.cmd`。
2. 打开 `db.go`，输入目标 profile 配置的激活关键词。
...
| `processName` | 进程名，不带 `.exe`；用于查找已运行实例和关闭辅助进程。 |
| `windowClass` | 可选的顶级窗口类名，必须精确匹配。多窗口程序应填写真正主窗口的类名；留空时使用第一个可见顶级窗口。 |
...
| `activationPhrase` | 在 `db.go` 中输入的启动关键词。识别后会立即从编辑器文本中移除。 |
| `workspaceDirectory` | Explorer 初始目录。相对路径必须位于宿主目录内，也可以填写绝对路径。 |
| `triggerFile` | 关键词监听文件。需要共用同一入口的 profile 都应填写 `db.go`。 |
